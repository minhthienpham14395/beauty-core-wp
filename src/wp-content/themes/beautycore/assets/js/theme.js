(function () {
  ...
  document.querySelectorAll("[data-homepage-booking-form]").forEach((form) => {
    const date = form.querySelector('[name="date"]');
    const time = form.querySelector('[name="start_time"]');
    const service = form.querySelector('[name="service_id"]');
    const today = form.dataset.today || "";
    const openingTime = form.dataset.openingTime || "09:00";
    const closingTime = form.dataset.closingTime || "21:00";
    const workdays = (form.dataset.workdays || "").split(",").filter(Boolean);
    ...
    const validateSchedule = () => {
      if (date) {
        ...
        if (date.value && today && date.value < today) {
          dateError = "Vui lòng chọn ngày từ hôm nay trở đi.";
        } else if (date.value && workdays.length) {
          const weekday = String(new Date(`${date.value}T00:00:00`).getDay());
          if (!workdays.includes(weekday)) dateError = "Beauty Core chưa nhận lịch vào ngày đã chọn.";
        }
        date.setCustomValidity(dateError);
      }
      ...
      if (time.value) {
        ...
        const duration = Number(selectedOption?.dataset.duration) || 30;
        const start = toMinutes(time.value);
        const opening = toMinutes(openingTime);
        const closing = toMinutes(closingTime);
        if (start < opening || start >= closing || start + duration > closing) {
          timeError = `Vui lòng chọn giờ trong khung ${openingTime}-${closingTime}, đủ thời lượng dịch vụ.`;
        }
      }
      time.setCustomValidity(timeError);
    };

    [date, time, service].filter(Boolean).forEach((field) => {
  ...
  document.querySelectorAll("[data-booking-notice]").forEach((notice) => {
    ...
    notice.querySelector("[data-booking-notice-close]")?.addEventListener("click", dismiss);
