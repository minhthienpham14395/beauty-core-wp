(function () {
  ...
  function appointmentFormFromRoot(root) {
    if (!root) {
      return null;
    }
    if (root.matches && root.matches('.beautycore-appointment-form')) {
      return root;
    }
    return root.querySelector ? root.querySelector('.beautycore-appointment-form') : null;
  }

  function refreshAppointmentAssignments(root) {
    var form = appointmentFormFromRoot(root || document);
    if (!form) {
      return;
    }
    ...
    var serviceId = service && service.value !== '0' ? service.value : '';
    ...
    if (staff) {
      ...
      Array.prototype.forEach.call(staff.options, function (option) {
        var unavailable = Boolean(option.value) && (
          (branchId && !optionSupports(option, 'data-branches', branchId)) ||
  ...
  function initAppointmentForm(root) {
    var form = appointmentFormFromRoot(root || document);
    if (!form || form.getAttribute('data-beautycore-appointment-form-ready') === 'true') {
      return;
    }

    form.setAttribute('data-beautycore-appointment-form-ready', 'true');
    refreshAppointmentAssignments(form);
  ...
  function initAppointmentResultsRefresh() {
    var results = document.querySelector('[data-beautycore-appointment-results]');
    if (!results || results.getAttribute('data-beautycore-refresh-ready') === 'true') {
      return;
    }

    results.setAttribute('data-beautycore-refresh-ready', 'true');
    ...
    function refreshResults() {
      ...
      fetch(window.location.href, {
        ...
        .then(function (response) {
          if (!response.ok) {
            throw new Error('Không thể tải lại lịch hẹn.');
          }
          return response.text();
        })
        .then(function (markup) {
          var parsedDocument = new DOMParser().parseFromString(markup, 'text/html');
          var nextResults = parsedDocument.querySelector('[data-beautycore-appointment-results]');
          if (nextResults) {
        ...
        .catch(function () {
          // The next notification or a manual reload can retry the update.
        })
        .finally(function () {
          refreshing = false;
        });
    ...
    document.addEventListener('beautycore:appointment-created', refreshResults);
    document.addEventListener('beautycore:appointment-poll', function (event) {
      var appointmentId = Number(event.detail && event.detail.appointmentId) || 0;
      if (!appointmentId) {
        return;
      }
  ...
  function initAppointmentModal() {
    ...
    initAppointmentForm(document);
    initAppointmentResultsRefresh();
