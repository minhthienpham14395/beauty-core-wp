...
function beautycore_appointment_payment_statuses() {
    return array(
...
function beautycore_appointment_data($post) {
    ...
    return array(
        ...
        'payment_status' => sanitize_key((string) beautycore_appointment_meta($post->ID, array('_beautycore_payment_status', 'payment_status'), 'unpaid')),
        'refund_amount'  => (float) beautycore_appointment_meta($post->ID, array('_beautycore_refund_amount', 'refund_amount'), 0),
...
function beautycore_appointment_normalize_data($data, $existing = array()) {
    ...
    if ($duration <= 0) {
        $duration = max(1, absint(apply_filters('beautycore_appointment_default_duration', 30, $data, $existing)));
    }
    ...
    $payment_status = isset($data['payment_status']) ? sanitize_key(wp_unslash($data['payment_status'])) : (isset($existing['payment_status']) ? $existing['payment_status'] : 'unpaid');
    if (!isset(beautycore_appointment_payment_statuses()[$payment_status])) {
        return new WP_Error('beautycore_invalid_payment_status', 'Trạng thái thanh toán không hợp lệ.');
    }
    $refund_amount = isset($data['refund_amount']) && $data['refund_amount'] !== '' ? (float) str_replace(',', '', sanitize_text_field(wp_unslash($data['refund_amount']))) : (isset($existing['refund_amount']) ? (float) $existing['refund_amount'] : 0);
    if ($refund_amount < 0 || $refund_amount > $price) {
        return new WP_Error('beautycore_invalid_refund', 'Khoản hoàn tiền phải từ 0 đến giá trị lịch hẹn.');
    }
...
function beautycore_create_appointment($data, $source = 'website', $appointment_id = 0) {
    ...
    $meta = array(
        ...
        '_beautycore_payment_status'      => $normalized['payment_status'],
        '_beautycore_refund_amount'       => $normalized['refund_amount'],
...
function beautycore_appointment_admin_assets($hook) {
    ...
    $script_path = get_theme_file_path('/assets/js/admin-appointment.js');
    $script_version = file_exists($script_path) ? (string) filemtime($script_path) : BEAUTYCORE_ADMIN_VERSION;
    wp_enqueue_script(
...
function beautycore_appointment_notification_assets() {
    if (!current_user_can('view_beautycore_schedule')) {
        return;
    }

    $script_path = get_theme_file_path('/assets/js/admin-appointment-notification.js');
    $script_version = file_exists($script_path) ? (string) filemtime($script_path) : BEAUTYCORE_ADMIN_VERSION;
    wp_enqueue_script(
        'beautycore-appointment-notification',
        get_theme_file_uri('/assets/js/admin-appointment-notification.js'),
        array(),
        $script_version,
        true
    );
    wp_localize_script('beautycore-appointment-notification', 'BEAUTYCORE_APPOINTMENT_NOTIFICATION', array(
        'ajaxUrl'      => admin_url('admin-ajax.php'),
        'nonce'        => wp_create_nonce('beautycore_appointment_notification'),
        'pollInterval' => 30000,
        'storageKey'   => 'beautycore_appointment_notification_' . get_current_user_id(),
    ));
}
add_action('admin_enqueue_scripts', 'beautycore_appointment_notification_assets');

function beautycore_ajax_appointment_notification() {
    check_ajax_referer('beautycore_appointment_notification');

    if (!current_user_can('view_beautycore_schedule')) {
        wp_send_json_error(array('message' => 'Bạn không có quyền xem lịch hẹn.'), 403);
    }
    ...
    wp_send_json_success(array(
        'latestId' => !empty($latest) ? absint($latest[0]) : 0,
    ));
}
add_action('wp_ajax_beautycore_appointment_notification', 'beautycore_ajax_appointment_notification');
...
function beautycore_render_appointment_admin_page() {
    ...
    beautycore_appointment_notification_assets();
    ...
    echo '</div><div><button type="button" class="button beautycore-appointment-sound-toggle" data-beautycore-appointment-sound aria-pressed="false"><span class="dashicons dashicons-controls-volumeon" aria-hidden="true"></span>Bật âm thanh</button></div><div class="beautycore-view-switcher">';
    ...
    echo '<div class="beautycore-appointment-results" data-beautycore-appointment-results>';
    if ($view === 'list') {
...
function beautycore_render_appointment_edit_page($appointment_id_override = null, $fragment = false) {
    ...
    echo '<section class="beautycore-panel"><h2>Thời gian và dịch vụ</h2><div class="beautycore-form-grid"><div class="beautycore-form-field"><label for="appointment_date"><strong>Ngày check-in *</strong></label><input id="appointment_date" type="date" name="date" required value="' . esc_attr(substr($start, 0, 10)) . '"></div><div class="beautycore-form-field"><label for="start_time"><strong>Giờ check-in *</strong></label><input id="start_time" type="time" name="start_time" required value="' . esc_attr(substr($start, 11, 5)) . '"></div><div class="beautycore-form-field"><label for="end_time"><strong>Giờ kết thúc</strong></label><input id="end_time" type="time" name="end_time" value="' . esc_attr(isset($appointment['end']) ? substr($appointment['end'], 11, 5) : '') . '"><p class="description">Để trống để hệ thống tự tính.</p></div></div><div class="beautycore-form-grid"><div class="beautycore-form-field"><label for="branch_id"><strong>Chi nhánh</strong></label><select id="branch_id" name="branch_id"><option value="">Chưa chọn</option>';
    ...
    echo '</select></div><div class="beautycore-form-field"><label for="service_id"><strong>Dịch vụ</strong></label><select id="service_id" name="service_id"><option value="0">Chọn dịch vụ</option>';
    ...
    echo '</select></div><div class="beautycore-form-field"><label for="duration"><strong>Thời lượng (phút)</strong></label><input id="duration" type="number" min="1" name="duration" value="' . esc_attr(isset($appointment['duration']) ? $appointment['duration'] : '') . '"><p class="description">Để trống để dùng 30 phút.</p></div><div class="beautycore-form-field"><label for="price"><strong>Giá</strong></label><input id="price" type="number" min="0" step="1000" name="price" value="' . esc_attr(isset($appointment['price']) ? $appointment['price'] : '') . '"></div></div></section>';
    echo '<section class="beautycore-panel"><h2>Nhân viên và ghi chú</h2><div class="beautycore-form-grid"><div class="beautycore-form-field"><label for="staff_id"><strong>Nhân viên</strong></label><select id="staff_id" name="staff_id"><option value="0">Chưa phân công</option>';
    ...
    echo '</select><p class="description">Chọn dịch vụ trước để lọc nhân viên phù hợp.</p></div></div><div class="beautycore-form-field"><label for="notes"><strong>Ghi chú</strong></label><textarea id="notes" name="notes" rows="4">' . esc_textarea(isset($appointment['notes']) ? $appointment['notes'] : '') . '</textarea></div></section></div>';
    ...
    echo '</select><div class="beautycore-form-field"><label for="payment_status"><strong>Thanh toán</strong></label><select id="payment_status" name="payment_status">';
    foreach (beautycore_appointment_payment_statuses() as $key => $label) {
        echo '<option value="' . esc_attr($key) . '" ' . selected(isset($appointment['payment_status']) ? $appointment['payment_status'] : 'unpaid', $key, false) . '>' . esc_html($label) . '</option>';
    }
    echo '</select></div><div class="beautycore-form-field"><label for="refund_amount"><strong>Hoàn tiền</strong></label><input id="refund_amount" type="number" min="0" step="1000" name="refund_amount" value="' . esc_attr(isset($appointment['refund_amount']) ? $appointment['refund_amount'] : '') . '"></div><p class="description">Doanh thu chỉ tính lịch Hoàn tất đã thanh toán, sau khi trừ khoản hoàn tiền.</p><p><button class="button button-primary" type="submit">Lưu lịch hẹn</button> <a class="button" href="' . esc_url(beautycore_appointment_relative_url(admin_url('admin.php?page=beautycore-appointments'))) . '">Quay lại</a></p></section>';
    if ($id) {
...
function beautycore_handle_public_create_appointment() {
    ...
    $referer = remove_query_arg(array('booking', 'booking_error'), wp_get_referer() ?: home_url('/'));
    $service_id = isset($_POST['service_id']) ? absint($_POST['service_id']) : 0;
    $date = isset($_POST['date']) ? sanitize_text_field(wp_unslash($_POST['date'])) : '';
    $start_time = isset($_POST['start_time']) ? sanitize_text_field(wp_unslash($_POST['start_time'])) : '';
    $service = get_post($service_id);
    $booking_enabled = $service_id && get_post_meta($service_id, '_beautycore_booking_enabled', true) !== '0';

    if (!$service || $service->post_type !== 'beautycore_service' || $service->post_status !== 'publish' || !$booking_enabled || !$date || !$start_time) {
        wp_safe_redirect($referer . '#dat-lich');
        exit;
    }
    ...
    wp_safe_redirect(add_query_arg('booking', 'success', $referer) . '#dat-lich');
    exit;
}
