...
$managed_reviews = function_exists('beautycore_get_approved_reviews') ? beautycore_get_approved_reviews(4) : array();
$reviews = array(
...
<section class="reviews-section" aria-labelledby="reviews-title"><div class="container">
    ...
    <div class="reviews-grid">
        <?php if ($managed_reviews) : ?>
            <?php foreach ($managed_reviews as $review) : ?>
                <article class="review-card review-card--text"><div class="review-card__content"><span class="review-card__stars" aria-label="<?php echo esc_attr($review['rating']); ?> trên 5 sao"><?php echo esc_html(str_repeat('★', $review['rating'])); ?></span><p><?php echo esc_html($review['content']); ?></p></div><div class="review-card__meta"><strong><?php echo esc_html($review['name']); ?></strong><span><?php echo esc_html(wp_date('d/m/Y', strtotime($review['date']), wp_timezone())); ?></span></div></article>
            <?php endforeach; ?>
        <?php else : ?>
