...
<section class="treatments-section" id="services">
    <div class="container">
                ...
                <section class="service-group" id="<?php echo esc_attr($group['id']); ?>" data-service-panel="<?php echo esc_attr($group['id']); ?>">
                    ...
                    <div class="service-list">
                        <?php foreach ($group['services'] as $service) : ?>
                            <article class="service-item"><div class="service-item-top"><div><h4><?php echo esc_html($service['name']); ?></h4><span class="service-duration"><?php echo esc_html($service['duration']); ?></span></div><strong><?php echo wp_kses_post(isset($service['price_html']) ? $service['price_html'] : esc_html($service['price'])); ?></strong></div><p><?php echo esc_html($service['details']); ?></p><a href="<?php echo esc_url(beautycore_booking_section_url()); ?>" class="service-booking" data-track="booking_click">Đặt lịch <span aria-hidden="true">→</span></a></article>
                        <?php endforeach; ?>
                    </div>
        ...
        <div class="service-note"><p>Thời lượng và quy trình có thể được điều chỉnh theo tình trạng thực tế. Vui lòng đặt lịch trước để được phục vụ tốt nhất.</p><a href="<?php echo esc_url(beautycore_booking_section_url()); ?>" class="btn btn-primary">Đặt lịch ngay</a></div>
    </div>
</section>
