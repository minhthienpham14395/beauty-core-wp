...
function beautycore_stage4_validate_appointment_resources($normalized) {
    ...
    if (!$branch_id && beautycore_stage4_branch_posts(false)) {
        return new WP_Error('beautycore_branch_required', 'Vui lòng chọn chi nhánh.');
    }
    ...
    if ($staff_id) {
        ...
        if ($staff['profile']) {
            ...
            if ($service_id && !in_array($service_id, $staff['service_ids'], true)) {
